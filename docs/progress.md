...
|---|---:|---|
| Infrastructure maturity | 45% | Required tree, exact bit-level `SAT-gamma` target, iterative parser/context/full-cofactor tests, semantic quotient specification, corrected open-target/bridge labels, literature/barrier audits, ledgers, claims registry, manifest, and cold-clone evidence exist. Formal foundations and broader independent review are incomplete. |
| Formally closed proof chain | 0% | No terminal-critical statement has a proof-assistant artifact. |
| Real progress toward P vs NP | 0.00% | The work detects that GATE-003 was equivalent to the desired non-uniform separation, closes padding/reindexing, fanout, contiguous-coverage, shared-core, parser-lifting, output cross-table, and constant-width cofactor pseudo-routes, and isolates a same-column internal collision obligation. It proves no new lower bound for SAT and supplies no polynomial SAT algorithm. |
