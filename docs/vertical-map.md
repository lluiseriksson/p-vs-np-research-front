...
LEMMA-194 spends the exhausted neutral budget to prove `fanout(g)=1`. This
...
LEMMA-196 shows that satisfying-row alignment does not extend globally: a
...
LEMMA-198 proves every handoff is sensitive to both fresh inputs and survives
...
LEMMA-200 gives necessary and sufficient rowwise identities for both aligned
masks and counterflows. A local two-route gadget realizes exact global
