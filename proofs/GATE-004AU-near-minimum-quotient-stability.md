...
Prove that some minimum circuit has total diagonal quotient size at least

`7m-2(Delta+K)`.

Here the quotient size is the number of distinct active nonconstant Boolean
functions obtained by restricting every gate separately to the two designated
base rows. This is the class-count convention used throughout GATE-004AE.
...
| Circuit size | Parent `S=K+6m-Delta` with `0<=Delta<=K`; target total diagonal quotient at least `7m-2(Delta+K)` |
