...
LEMMA-101 invalidates the original one-block-length translation argument.
LEMMA-106 supplies the corrected extension: identifiers 1 through 68 already
realize every ordinary quartet mask whenever any gap is at least 72. It uses
the exact pair/triple component budgets in LEMMA-104/105 and blocks of maximum
length 36. Therefore every possible obstruction has all three gaps in
`{1,...,71}`, and it suffices to check

`4 * 71^3 = 1,431,644`

types. A length-360 representative leaves at least 68 coordinates at both
boundaries.
...
This proves GATE-004AD only as a witness-family theorem. No circuit loss, lower
