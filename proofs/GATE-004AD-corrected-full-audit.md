...
`r in {0,1,2,3}` and each `gi in {1,...,139}`.

By LEMMA-102, a zero-failure result on all
`4*139^3=10,742,476` types, together with the existing boundary and long-
...
splits the domain into 556 one-first-gap shards of 19,321 types each. Every
...
certificate only when all 10,742,476 types are covered with zero failures.
...
output, and merge only after all 556 jobs finish. A typical shard command is
...
| Asymptotic quantifiers | Exact finite domain of 10,742,476 reduced types; LEMMA-102 supplies the all-interior reduction if zero failures are proved |
