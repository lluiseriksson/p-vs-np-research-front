...
`{0,1}`, and the final canonical deficit is at most `K`.
...
Thus monotonicity, the unit increment cap, and the final `O(K)` budget do
...
| Circuit size | `Delta_0=0`, increments in `{0,1}`, endpoint at most `K`; late unit jump remains allowed |
